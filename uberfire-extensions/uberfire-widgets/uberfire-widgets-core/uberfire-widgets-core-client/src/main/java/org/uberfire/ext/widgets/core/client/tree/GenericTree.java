...
public class GenericTree<T extends GenericTreeItem> extends Composite implements HasSelectionHandlers<T>,
                                                      HasOpenHandlers<T>,
                                                      HasCloseHandlers<T> {
    ...
    public GenericTreeItem getSelectedItem() {
        return curSelection;
    }
    ...
    public GenericTreeItem addItem(final GenericTreeItem.Type type,
                                   final String value,
                                   final String label,
                                   final IsWidget icon) {
        final GenericTreeItem item = new GenericTreeItem(type,
                                                         value,
                                                         label,
                                                         icon);
        return addItem(item);
    ...
    public GenericTreeItem addItem(final GenericTreeItem item) {
        container.add(item);
        item.setGenericTree(this);
        return item;
    }

    public void removeItem(final GenericTreeItem genericTreeItem) {
        container.remove(genericTreeItem);
    }

    public Iterable<GenericTreeItem> getGenericItems() {
        return new Iterable<GenericTreeItem>() {
            @Override
            public Iterator<GenericTreeItem> iterator() {
                return new GenericTreeItem.TreeItemIterator(container);
            }
        };
    }
