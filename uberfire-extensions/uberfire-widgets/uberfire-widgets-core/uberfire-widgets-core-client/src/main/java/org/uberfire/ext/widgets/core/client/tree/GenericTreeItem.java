...
public class GenericTreeItem extends Composite {

    private final Type type;
    private GenericTree genericTree;
    private Object userObject;
    private GenericTreeItem parent;
    ...
    public GenericTreeItem(final Type type,
                           final String value,
                           final String label,
                           final IsWidget icon) {
        this.label = label;
        ...
        if (type.equals(Type.CONTAINER) || type.equals(Type.ROOT) || type.equals(Type.FOLDER)){
            ...
            item.setStylePrimaryName(TreeNavigatorResources.INSTANCE.css().treeItem());
            {
                ...
                item.addDomHandler(new ClickHandler() {
                                       @Override
                                       public void onClick(ClickEvent event) {
                                           genericTree.onSelection(GenericTreeItem.this,
                                                                   true);
                                       }
                                   },
                                   ClickEvent.getType());
    ...
    public void setState(final State state,
        ...
        if (!this.state.equals(state)) {
            ...
            if (fireEvents && genericTree != null) {
                genericTree.fireStateChanged(this,
                                             state);
            }
        }
    ...
    public GenericTreeItem addItem(final Type type,
                                   final String value,
                                   final String label,
                                   final IsWidget icon) {
        if (notFolder()) {
        ...
        final GenericTreeItem child = makeChild(type,
                                                value,
                                                label,
                                                icon);
        content.add(child);
        child.setGenericTree(genericTree);
    ...
    protected GenericTreeItem makeChild(final Type type,
                                        final String value,
                                        final String label,
                                        final IsWidget icon) {
        return new GenericTreeItem(type,
                                   value,
                                   label,
                                   icon);
    }
    ...
    public GenericTreeItem getChild(final int i) {
        if (i + 1 > content.getWidgetCount()) {
            return null;
        }
        return (GenericTreeItem) content.getWidget(i);
    }

    public Iterable<GenericTreeItem> getGenericChildren() {
        return new Iterable<GenericTreeItem>() {
            @Override
            public Iterator<GenericTreeItem> iterator() {
                return new TreeItemIterator(content);
            }
        };
    }

    void setGenericTree(final GenericTree genericTree) {
        this.genericTree = genericTree;
    }

    void setParent(final GenericTreeItem parent) {
        this.parent = parent;
    }
    ...
    public void remove() {
        ...
        } else if (genericTree != null) {
            // If the item has no parent, but is in the Tree, it must be a top-level
            // element.
            genericTree.removeItem(this);
        }
    }

    public void removeItem(final GenericTreeItem genericTreeItem) {
        content.remove(genericTreeItem);
    }
